angular.module("mainApp")
    .controller('viewBooksController', ["$scope", "viewBooksService", "crudService",function($scope, viewBooksService, crudService) {
        ...
        function filterQuery() {
            ...
            let promise = viewBooksService.filterQuery(self.book_IDFilter, self.bookFilter);
        ...
        function deleteSelected() {
            ...
            for(var x=0; x<Object.keys(self.selectedRows).length; x++) {
                ...
                if(self.selectedRows[keys[x]]) {
                    ...
                    let promise = crudService.dbDelete('books', 'book_ID', keys[x]);
        ...
        function saveSelected() {
            ...
            let promise = crudService.dbUpdate('books', 'book_ID', self.activeRow.book_ID, self.activeRow);
        ...
        function getTemplate(row) {
            ...
            if(row.book_ID === self.activeRow.book_ID) {
        ...
        function initializeSelectedRows(rows) {
            for(var x=0; x<rows.length; x++) {
                self.selectedRows[rows[x].book_ID] = false;
            }
        }
        ...
        function changeSelectionStatus(row) {
            ...
            if(row.book_ID != self.editableRow.book_ID) {
                if(self.selectedRows[row.book_ID] == true) {
                    self.selectedRows[row.book_ID] = false;
                    self.allToggled = false;
                }
                else {
                    self.selectedRows[row.book_ID] = true;
