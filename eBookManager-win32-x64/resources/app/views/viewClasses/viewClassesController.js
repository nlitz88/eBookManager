angular.module("mainApp")
    .controller('viewClassesController', ["$scope", "viewClassesService", "crudService", function($scope, viewClassesService, crudService) {
        ...
        function filterQuery() {
            ...
            let promise = viewClassesService.filterQuery(self.class_IDFilter, self.classFilter);
        ...
        function deleteSelected() {
            ...
            for(var x=0; x<Object.keys(self.selectedRows).length; x++) {
                ...
                if(self.selectedRows[keys[x]]) {
                    ...
                    let promise = crudService.dbDelete('classes', 'class_ID', keys[x]);
        ...
        function saveSelected() {
            ...
            let promise = crudService.dbUpdate('classes', 'class_ID', self.activeRow.class_ID, self.activeRow);
        ...
        function getTemplate(row) {
            ...
            if(row.class_ID === self.activeRow.class_ID) {
        ...
        function initializeSelectedRows(rows) {
            for(var x=0; x<rows.length; x++) {
                self.selectedRows[rows[x].class_ID] = false;
            }
        }
        ...
        function changeSelectionStatus(row) {
            ...
            if(row.class_ID != self.editableRow.class_ID) {
                if(self.selectedRows[row.class_ID] == true) {
                    self.selectedRows[row.class_ID] = false;
                    self.allToggled = false;
                }
                else {
                    self.selectedRows[row.class_ID] = true;
